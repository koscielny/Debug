...
returns shade
*/
vec3 Triangle::shade(const Intersection& hit, TreeNode* tree, bool single_ray) {//single_ray:对光源单采样，计算间接光照用，计算直接光照不用
...
//	}
	...
	if (single_ray){
		...
		Ray ray = Ray(hit.point + EPSILON*s_norm, dir);//+ EPSILON * s_norm
		Intersection light_hit = tree->intersect(ray);//shadow_ray
	...
	for (int i=0; i<num_samples; i+=1){
		...
		vec3 dir = glm::normalize(light_samp - hit.point);
		Ray ray = Ray(hit.point + EPSILON*s_norm, dir);//+ EPSILON * s_norm//s_norm可能朝背面
...
vec3 Sphere::shade(const Intersection& hit, TreeNode* tree, bool derp) {
	cout << "THIS IS NOT IMPLEMENTED" << endl;
	exit(1);
}

...
vec3 AreaLight::shade(const Intersection& intersect, TreeNode* tree, bool single_ray)
{
	return shapelight->shade(intersect, tree, single_ray);
}
...
vec3 DirectionalLight::shade(const Intersection& hit, TreeNode* tree, bool single_ray){
	vec3 normal = hit.primative->getNormal(hit);
	if(!isVisible(hit.point+EPSILON*normal,tree)) {
		return vec3(0.0,0.0,0.0);
	}
	vec3 shade = max(0.0, glm::dot(normal, -direction)) * hit.primative->diffuse;// = vec3(0,0,0);//
...
vec3 PointLight::shade(const Intersection& hit, TreeNode* tree, bool single_ray){
	vec3 normal = hit.primative->getNormal(hit);
...
bool PointLight::isVisible(const vec3& p, TreeNode* tree) {
	...
	Intersection lighthit = tree->intersect(ray);
	if (lighthit.primative){
		if (glm::distance(p, lighthit.point) < dist){
			return false;
		}
	}
	return true;

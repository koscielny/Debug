...
/* Shoot ray at scene */
vec3 findColor(Scene* scene, Ray& ray, double weight, vector<Intersection> * pointlist) {
	...
	*********************************************/
	...
	if (u1 < threshold1) {//计算漫反射//反射率为零则不搞反射，折射率为零则不搞折射 || glm::length(hit.primative->specular)<EPSILON
		...
		newRay = Ray(hit.point + 3 * EPSILON*normal, newDirection);
		double prob = 1.0/threshold1;
		color += prob * hit.primative->diffuse * findColor(scene, newRay, weight*threshold1, pointlist);
		...
		//multiplier *= dot;	//封印
		color += multiplier * hit.primative->specular * findColor(scene, newRay, (1.0 - threshold1)*threshold2*weight, pointlist);
		...
		//cout << refract.x;
		{
			newRay = Ray(hit.point - 3*EPSILON*normal, refract);
			...
			color += transparency * multiplier * findColor(scene, newRay, weight*(1.0 - threshold1)*(1 - threshold2), pointlist);
		}
		//color += findColor(scene, newRay, specWeight*weight);
		...
		for (int i=0; i<scene->width; i++) {
			...
			for(double a=0; a<subdivisions; a+=1) {
				for(double b=0; b<subdivisions; b+=1) {
					...
					color += findColor(scene, ray, 1.0, NULL);
				}
			}
...
void draw_ray_debug(int i, int j)
{
	...
	color += findColor(scene, ray, 1.0, &pointlist);
...
//	glBegin(GL_LINE_LOOP);
	...
	{
		glVertex3d(pointlist[ii].point.x, pointlist[ii].point.y, pointlist[ii].point.z);
		cout << pointlist[ii].point.x << " " << pointlist[ii].point.y << " " << pointlist[ii].point.z << endl;
	}
	glEnd();
